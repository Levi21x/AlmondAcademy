...
function daysRemainingLabel(examDate: string) {
  ...
  if (days <= 3) return `${days} days remaining - critical`;
  if (days <= 7) return `${days} days remaining - urgent`;
...
export function CrisisContent() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load crisis status");
    } finally {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => setLoadingStepIndex((p) => Math.min(p + 1, LOADING_STEPS.length - 1)), 2000);
    return () => window.clearInterval(id);
  }, [isActivating]);

  const todayData = useMemo(() => {
    if (!session?.crisis_plan?.days?.length) return null;
    return session.crisis_plan.days.find((day) => {
      const completed = (session.topic_progress || []).filter((e) => e.day_number === day.day && e.is_completed).length;
      return completed < day.hours.length;
    }) ?? session.crisis_plan.days[0];
  }, [session]);

  const todayProgress = useMemo(() => {
    if (!session || !todayData) return { completed: 0, total: 0, percent: 0 };
    const total = todayData.hours.length;
    const completed = (session.topic_progress || []).filter((e) => e.day_number === todayData.day && e.is_completed).length;
  ...
  const activate = async () => {
    ...
    try {
      ...
      setSession(created);
      setStatus((prev) => prev ? { ...prev, active_session: created, can_activate: prev.is_premium, free_activation_used: prev.is_premium ? prev.free_activation_used : true } : prev);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to activate crisis mode");
    } finally {
  ...
  const openTeach = async (block: CrisisHourBlock) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load teaching content");
    } finally {
  ...
  const markComplete = async (dayNumber: number, topicName: string, isCompleted: boolean) => {
    ...
    try {
      ...
      const token = authSession?.access_token ?? fallbackToken;
      if (!token) throw new Error("Please sign in again.");
      await updateTopicProgress(token, session.id, { day_number: dayNumber, topic_name: topicName, is_completed: isCompleted });
      await loadStatus();
    } catch (err) {
  ...
  const doRecalibrate = async () => {
    ...
    try {
      ...
      const token = authSession?.access_token ?? fallbackToken;
      if (!token) throw new Error("Please sign in again.");
      const updated = await recalibrateSession(token, session.id, { available_hours_per_day: recalibrateHours, preparation_level: recalibratePrep });
      setSession(updated);
      setRecalibrateOpen(false);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to recalibrate");
    }
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-10rem)] space-y-4">
      {locked ? (
        <div className="absolute inset-0 z-20 flex items-center justify-center rounded-2xl border border-[#5a2f2a] bg-[#131313]/95 p-6">
          <div className="w-full max-w-xl rounded-2xl border border-[#5a2f2a] bg-[#1f1f1f] p-8 text-center">
            ...
            <h2 className="aa-h2 mt-3 text-[var(--aa-text-1)]">Crisis Mode — Premium Feature</h2>
            <p className="mt-2 text-sm text-[#cec5b9]">You have used your free Crisis Mode activation.</p>
            <p className="text-sm text-[#cec5b9]">Upgrade for unlimited access.</p>
      ...
      {!session ? (
        <section className="rounded-2xl border border-[#5a2f2a] bg-[#131313] p-5 md:p-8">
          <div className="mb-4 flex items-center gap-2 text-[#ffcf9d]">
            ...
            <h2 className="aa-h2">Crisis Mode</h2>
          </div>
          ...
          {status && !status.free_activation_used ? (
            <div className="mb-4 rounded-xl border border-green-700/60 bg-green-900/20 px-3 py-2 text-sm text-green-300">
            ...
            {!isActivating ? (
              <div className="space-y-4">
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    {PREP_OPTIONS.map((opt) => (
                      <button
                        key={opt.key}
                        type="button"
                        onClick={() => setPreparationLevel(opt.key)}
                        className={`rounded-xl border p-3 text-left ${preparationLevel === opt.key ? "border-[#ffcf9d] bg-[#ffcf9d]/10" : "border-[#4c463d] bg-[#131313]"}`}
                      >
                        <p className="text-sm font-semibold text-[#fff2de]">{opt.title}</p>
                        <p className="text-xs text-[#cec5b9]">{opt.description}</p>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="range" min={4} max={16} step={1} value={hoursPerDay}
              ...
              <div className="space-y-4 text-center">
                ...
                <div className="space-y-2 text-left">
                  {LOADING_STEPS.map((step, i) => {
                    const visible = i <= loadingStepIndex;
                    ...
                    return (
                      <div key={step} className={`flex items-center gap-2 text-sm transition ${visible ? "opacity-100" : "opacity-30"}`}>
                        {complete ? <Check className="h-4 w-4 text-green-300" /> : <span className="h-2 w-2 animate-pulse rounded-full bg-[#ffcf9d]" />}
        ...
        <section className="space-y-4">
          ...
          {todayData ? (
            <div className="rounded-xl border border-[#5a2f2a] bg-[#1f1f1f] p-5">
              ...
              <div className="mt-4 space-y-3">
                {todayData.hours.map((block, i) => {
                  const done = (session.topic_progress || []).some(
                    (e) => e.day_number === todayData.day && e.topic_name === block.topic && e.is_completed,
                  );
                  return (
                    <div key={`${block.topic}-${i}`} className="rounded-lg border border-[#4c463d] bg-[#131313] p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <input type="checkbox" checked={done} onChange={(e) => void markComplete(todayData.day, block.topic, e.target.checked)} className="h-4 w-4 accent-[#ffcf9d]" />
              ...
              <div className="mt-4">
                <p className="text-xs text-[#cec5b9]">{todayProgress.completed}/{todayProgress.total} topics done today</p>
                <div className="mt-1 h-2 rounded-full bg-[#353534]">
                  <div className="h-2 rounded-full bg-[#ffcf9d] transition-all" style={{ width: `${todayProgress.percent}%` }} />
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-[#5a2f2a] bg-[#1f1f1f] p-4">
              <h4 className="font-semibold text-[#ffcf9d]">Skip these — unlikely to appear</h4>
              <ul className="mt-2 space-y-1 text-sm text-[#cec5b9]">
                {(session.crisis_plan.what_to_skip || []).map((item) => <li key={item}>- {item}</li>)}
              </ul>
            </div>
            <div className="rounded-xl border border-[#7a3f30] bg-[#1f1f1f] p-4">
              ...
              <ul className="mt-2 space-y-1 text-sm text-[#fff2de]">
                {(session.crisis_plan.must_know || []).map((item) => <li key={item}>- {item}</li>)}
              </ul>
            </div>
          </div>
            ...
            <details open>
              ...
              <div className="mt-3 space-y-3">
                {(session.crisis_plan.days || []).map((day) => {
                  const done = (session.topic_progress || []).filter((e) => e.day_number === day.day && e.is_completed).length;
                  return (
                    <details key={day.day} className={`rounded-lg border p-3 ${todayData?.day === day.day ? "border-[#ffcf9d]/60" : "border-[#4c463d]"}`}>
                      <summary className="cursor-pointer text-sm text-[#e5e2e1]">
                        Day {day.day} — {day.theme} ({done}/{day.hours.length})
                      </summary>
                      <div className="mt-2 space-y-1 text-sm text-[#cec5b9]">
                        {day.hours.map((block, i) => <p key={`${day.day}-${i}`}>{block.time_block} — {block.subject}: {block.topic}</p>)}
                      </div>
                    </details>
                  );
                })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-4xl rounded-2xl border border-[#5a2f2a] bg-[#1f1f1f] p-5">
            ...
            <div className="mt-3 flex gap-2">
              {(["explanation", "key-points", "exam-tips"] as TeachTab[]).map((tab) => (
                <button
                  key={tab}
                  type="button"
                  onClick={() => setTeachTab(tab)}
                  className={`rounded-full px-3 py-1 text-xs ${teachTab === tab ? "bg-[#ffcf9d] text-[#3f2a16]" : "border border-[#4c463d] text-[#cec5b9]"}`}
                >
                  {tab === "explanation" ? "Explanation" : tab === "key-points" ? "Key Points" : "Exam Tips"}
                </button>
              ))}
            </div>
            <div className="mt-4 max-h-[55vh] overflow-y-auto rounded-xl border border-[#353534] bg-[#131313] p-4 text-sm text-[#e5e2e1]">
              ...
              {!teachingLoading && teachTab === "key-points" ? (
                <ol className="list-decimal space-y-2 pl-5">
                  {(teachingContent?.what_to_remember || []).map((item) => <li key={item}>{item}</li>)}
                </ol>
              ) : null}
              {!teachingLoading && teachTab === "exam-tips" ? (
                <ul className="list-disc space-y-2 pl-5">
                  {(teachingContent?.exam_questions || []).map((item) => <li key={item}>{item}</li>)}
                </ul>
              ) : null}
            </div>
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
              <button
                type="button"
                onClick={() => {
                  if (todayData && teachModal.block) void markComplete(todayData.day, teachModal.block.topic, true);
              ...
              <Link href={`/ai-tutor?subject=${encodeURIComponent(teachModal.block?.subject || "")}&prompt=${encodeURIComponent(`Teach me ${teachModal.block?.topic || ""} for my exam`)}`} className="text-sm text-[#ffcf9d]">
                Ask AlmondAI
              </Link>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-xl border border-[#5a2f2a] bg-[#1f1f1f] p-5">
            ...
            <label className="mt-4 block text-sm text-[#cec5b9]">
              ...
              <input type="range" min={4} max={16} value={recalibrateHours} onChange={(e) => setRecalibrateHours(Number(e.target.value))} className="mt-1 w-full accent-[#ffcf9d]" />
            </label>
            <label className="mt-3 block">
              <span className="text-sm text-[#cec5b9]">Preparation level now</span>
              <select
                value={recalibratePrep}
                onChange={(e) => setRecalibratePrep(e.target.value as PreparationLevel)}
                className="mt-1 w-full rounded-lg border border-[#4c463d] bg-[#131313] px-3 py-2 text-sm text-[#e5e2e1]"
              >
                {PREP_OPTIONS.map((opt) => <option key={opt.key} value={opt.key}>{opt.title}</option>)}
              </select>
            </label>
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setRecalibrateOpen(false)} className="rounded-lg border border-[#4c463d] px-3 py-2 text-sm text-[#cec5b9]">Cancel</button>
      ...
      {error ? (
        <div className="fixed bottom-4 right-4 z-50 rounded-lg border border-[#7a3f30] bg-[#2c1d1b] px-3 py-2 text-sm text-[#ffcf9d]">
          <div className="flex items-center gap-2">
